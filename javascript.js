...
.controller('portalcrowdmarkCtrl', ['$scope', '$http', '$q', 'portalcrowdmarkFactory', function ($scope, $http, $q, portalcrowdmarkFactory) {

    // Import variables and functions from service
    $scope.data = portalcrowdmarkFactory.data;
	$scope.items = portalcrowdmarkFactory.items;
	$scope.detailsItem = portalcrowdmarkFactory.detailsItem;
	
    // initialize the service
    portalcrowdmarkFactory.init($scope);

	// Show main view in the first column
	$scope.portalHelpers.showView('portalcrowdmarkMain.html', 1);
	
	// This function gets called when user clicks an item in the list
	$scope.showDetails = function(item){
		// Make the item that user clicked available to the template
		$scope.detailsItem.value = item;		
		$scope.portalHelpers.showView('portalcrowdmarkDetails.html', 2);
	}
	
}])
// Factory maintains the state of the widget
.factory('portalcrowdmarkFactory', ['$http', '$rootScope', '$filter', '$q', function ($http, $rootScope, $filter, $q) {
...
.directive('portalcrowdmarkDirectiveName', ['$http', function ($http) {
	return {
...
.filter('portalcrowdmarkFilterName', function () {
